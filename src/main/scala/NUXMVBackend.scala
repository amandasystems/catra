...
import java.io.{File, PrintWriter, OutputStreamWriter, InputStreamReader,
                BufferedReader}
...
class NUXMVBackend(instance : Instance) {

  import instance._
  import NUXMVBackend._
  ...
  val blockNum =
    automata.size
  val automataStates =
    for (auts <- automata) yield (for (aut <- auts) yield aut.states.toIndexedSeq)
  val stateNums =
  ...
  val blockVar =
    "block"
  val counterVars =
    (for ((cnt, n) <- counters.zipWithIndex) yield (cnt -> ("counter_" + n))).toMap
  ...
  private def inLabel(t : String, l : SymbolicLabel) : String = l match {
    ...
    case SymbolicLabel.CharRange(lower, upper) => and("" + lower.toInt + " <= " + t,
                                                      "" + t + " <= " + upper.toInt)
  ...
  println("VAR")
  println("  " + blockVar + " : " + intEnumType(automataStates.size + 1) + ";")
  ...
  for (((v, aut), states) <-
       stateVars.flatten zip automata.flatten zip automataStates.flatten)
  ...
  val autTransitions =
    for (blockId <- 0 until blockNum) yield {
      ...
      and(
        eq(blockVar, blockId),
        unchanged(blockVar),
        and((for ((vars, n) <- stateVars.zipWithIndex;
                  if n != blockId;
                  v <- vars)
             yield unchanged(v)) : _*),
        and((for ((counters, n) <- automataCounters.zipWithIndex;
                  if n != blockId;
                  cs <- counters; c <- cs)
        ...
        and((for (autId <- 0 until blockAutomata.size) yield {
               ...
               or((for (trip@(s1, l, s2) <- aut.transitions.toSeq;
                        offsets = transitionToOffsets(trip))
                   yield and(eq(stateVar, states indexOf s1),
                             eq(next(stateVar), states indexOf s2),
                             inLabel(inputVar, l),
                             and((for (c <- counters) yield (offsets get c) match {
                                    case Some(offset) =>
                                      eq(next(counterVars(c)),
                                         counterVars(c) + " + " + offset)
                                    case None =>
  ...
  val blockTransitions =
    for (blockId <- 0 until blockNum) yield {
      and(
        ...
        and((for (((v, aut), states) <-
                  stateVars(blockId) zip automata(blockId) zip automataStates(blockId))
             yield or((for (s <- states; if aut isAccept s)
                       yield eq(v, states indexOf s)) : _*)) : _*),
        if (blockId == blockNum - 1)
          and((for (c <- constraints)
