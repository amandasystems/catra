 ...
 */
class IterativeQuantifierElimProver(
    theories: Seq[Theory],
    baseOrder: TermOrder
) {
  ...
  private val goalSettings = {
    ...
    s = Param.THEORY_PLUGIN
      .set(s, PluginSequence(for (t <- theories; p <- t.plugin.toSeq) yield p))
  ...
  def apply(inputFor: ap.terfor.Formula): Conjunction = {
    val order = baseOrder
    //-BEGIN-ASSERTION-/////////////////////////////////////////////////////////
    Debug.assertPre(
      IterativeQuantifierElimProver.AC,
      ((Conjunction collectQuantifiers inputFor) subsetOf Set(Quantifier.ALL)) &&
        (order isSortingOf inputFor)
    )
    //-END-ASSERTION-///////////////////////////////////////////////////////////
    val goal = Goal(
      List(Conjunction.conj(inputFor, order)),
      Set(),
      Vocabulary(order),
      goalSettings
    )
...
//      println("found goal that cannot be extended")
      ...
      val allForsDisjunction =
        Conjunction.disj(
          tree.closingConstraint :: collectedConstraints,
          tree.order
        )
